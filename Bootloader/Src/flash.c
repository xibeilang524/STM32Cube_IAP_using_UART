...
env_t* env_load(void)
{
    ...
    FLASH_Read(ENV_ADDRESS, (uint16_t *)&env, sizeof(env_t) >> 1);
    if(env.fearure != ENV_FEATURE)
    {
        env.fearure  = ENV_FEATURE;
        env.filesize = 0;
        for(i = 0; i < sizeof(env.filename); i++)
        {
            env.filename[i] = 0;
        }
        FLASH_Write(ENV_ADDRESS, (uint16_t *)&env, sizeof(env_t) >> 1);
    }
...
int env_save(void)
{
    if(FLASH_Write(ENV_ADDRESS, (uint16_t *)&env, sizeof(env_t) >> 1) != sizeof(env_t))
        return 1;
    return 0;
}
